...
grn_module = Blueprint('grn', __name__)
db.create_all()
CORS(app)

@app.route('/employee-sites/<emp_id>', methods=['GET'])
@cross_origin()
def employee_sites(emp_id=None):
    if request.method == "GET":
        try :
            print(type(emp_id))
            ResultSet=db.session.query(Employee).filter(Employee.employee_id == emp_id).all()
            ...
            for record in ResultSet:
                ...
                'employee_sites' : record.emp_sites,}
...
@app.route('/employee-sites-vendor/<site>', methods=['GET'])
@cross_origin()
def employee_sites_vendor(site=None):
    if request.method == "GET":
        try :
            siteno = ''
            site = db.session.query(Site).filter(Site.site_name == site).all()
            for n in site:
                siteno = n.id
            print(siteno)
            ResultSet=db.session.query(PO).filter(PO.po_site == siteno).all()
            total_records = len(ResultSet)
            print(len(ResultSet))
            data = []
            sites = []
            ...
            for record in ResultSet:
                item = record.po_vendor
                if item not in seen:
                    seen.add(item)
                    sites.append(item)
            for v in sites:
                print(v)
                Vendors=db.session.query(Vendor).filter(Vendor.vendor_id == v).all()
                for name in Vendors:
                    vendor.append(name.vendor_name)
...
def rc_vendor_update(req_id = None):
    if request.method == "PATCH":
        ...
        ResultSet=db.session.query(RateComparative).filter(RateComparative.rate_comp_requisition_id == req_id).update({'rate_comp_vendors_list' : rate_comp_vendors_list, 'update_by' : update_by, 'rate_comp_status' : rate_comp_status},synchronize_session=False)
        try:
            db.session.commit()
                        ...
                        for a in empp:
                                if n.created_by == int(empid):
                                    ...
                                    records += 1
                                        ...
                                        data.append(record_dict)
                                        records += 1
                                elif int(rc_rank) == int(fRank[0]):
                                        if int(a.emp20) == int(fRank[1]):
                                            record_dict = {
                                            'rate_comp_requisition_id' : n.rate_comp_requisition_id,
                                            'rate_comp_requisition_title' : n.rate_comp_requisition_title,
                                            'rate_comp_urgent_requirement' : n.rate_comp_urgent_requirement,
        ...
        try :
            ...
            for n in reqq:
                ...
                for a in empp:
                    if n.created_by == int(empid):
                        records += 1
                    elif a.emp20 > int(rc_rank):
                        records += 1
                    elif int(a.emp20) == '100':
                        records += 1
            resp = {
                "pending_rc": records,
            }
    ...
    if request.method == "PUT":
        ...
        for record in ResultSet:
            ...
            for x in range(len(rate_comp_vendors_list)):
                ...
                purchaseOrder = PO(po_vendor_json = sanctioned_json, po_org = '1', po_site = site_id, po_status = 'pending', po_vendor = vendor_id, po7 = payment_terms, po6 = terms_and_conditions, po_vendor_name = record.ratecomp_1)
